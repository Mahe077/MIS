...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'unetMM':
        netG = UnetGeneratorMMU(input_nc, output_nc, 8, ngf,
                                norm_layer=norm_layer, use_dropout=use_dropout,  gpu_ids=gpu_ids)

    elif which_model_netG == 'resnetMM':
        netG = ResnetGeneratorMM(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                 use_dropout=use_dropout, n_blocks=9, gpu_ids=gpu_ids)
    elif which_model_netG == 'resnetMMReverse':
        netG = ResnetGeneratorMMReverse(
            output_nc, input_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9, gpu_ids=gpu_ids)
...
class ResnetGeneratorMMReverse(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect'):
        ...
        for i in range(pre_l_blocks, n_blocks):
            model_post1 += [ResnetBlock(ngf * mult, padding_type=padding_type,
                                        norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
            model_post2 += [ResnetBlock(ngf * mult, padding_type=padding_type,
                                        norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
...
class ResnetGeneratorMM(nn.Module):
    def __init__(self, input_nc=1, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect'):
        ...
        for i in range(pre_f_blocks):
            model1 += [ResnetBlock(ngf * mult, padding_type=padding_type,
                                   norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        for i in range(pre_l_blocks, n_blocks):
            model_post += [ResnetBlock(ngf * mult, padding_type=padding_type,
                                       norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        input_nc2 = 3
        model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc2, ngf, kernel_size=7,
                           padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
...
class UnetGeneratorMM(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        unet_block = UnetSkipConnectionBlock(
            ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlock(
            output_nc, ngf, input_nc=input_nc, submodule=unet_block, norm_layer=norm_layer, outermost=True)
...
class UnetGeneratorMMU(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        self.en1 = EnLayerBi(output_nc, ngf, input_nc=input_nc,
                             norm_layer=norm_layer, outermost=True)  # 128
        self.t11 = TransLayer(output_nc, ngf, input_nc=input_nc,
                              norm_layer=norm_layer, outermost=True, factor=2)
        self.t12 = TransLayer(output_nc, ngf, input_nc=input_nc,
                              norm_layer=norm_layer, outermost=True, factor=2)
        self.en2 = EnLayerBi(ngf, ngf * 2, input_nc=None,
                             norm_layer=norm_layer)  # 64
        self.t21 = TransLayer(ngf, ngf * 2, input_nc=None,
                              norm_layer=norm_layer, factor=2)
        self.t22 = TransLayer(ngf, ngf * 2, input_nc=None,
                              norm_layer=norm_layer, factor=2)
        self.en3 = EnLayerBi(ngf * 2, ngf * 4, input_nc=None,
                             norm_layer=norm_layer, factor=1)  # 32
        self.t31 = TransLayer(ngf * 2, ngf * 4, input_nc=None,
                              norm_layer=norm_layer, factor=2)
        self.t32 = TransLayer(ngf * 2, ngf * 4, input_nc=None,
                              norm_layer=norm_layer, factor=2)
        self.en4 = EnLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 16
        # self.t41 = TransLayer(ngf * 4, ngf * 8, input_nc=None, norm_layer=norm_layer, factor=2)
        self.t4 = TransLayer(ngf * 8, ngf * 8, input_nc=None,
                             norm_layer=norm_layer, factor=2)
        self.en5 = EnLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer, use_dropout=use_dropout)  # 8
        self.t5 = TransLayer(ngf * 8, ngf * 8, input_nc=None,
                             norm_layer=norm_layer, use_dropout=use_dropout)
        self.en6 = EnLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 4
        self.t6 = TransLayer(
            ngf * 8, ngf * 8, input_nc=None, norm_layer=norm_layer)
        self.en7 = EnLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 2
        self.t7 = TransLayer(
            ngf * 8, ngf * 8, input_nc=None, norm_layer=norm_layer)
        self.en8 = EnLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer, innermost=True)  # 1
        self.t8 = TransLayer(ngf * 8, ngf * 8, input_nc=None,
                             norm_layer=norm_layer, innermost=True)
        self.de8 = DeLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer, innermost=True)  # 2
        self.de7 = DeLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 4
        self.de6 = DeLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 8
        self.de5 = DeLayer(ngf * 8, ngf * 8, input_nc=None,
                           norm_layer=norm_layer, use_dropout=use_dropout)  # 16
        self.de4 = DeLayer(ngf * 4, ngf * 8, input_nc=None,
                           norm_layer=norm_layer)  # 32
        self.de3 = DeLayer(ngf * 2, ngf * 4, input_nc=None,
                           norm_layer=norm_layer)  # 64
        self.de2 = DeLayer(ngf, ngf * 2, input_nc=None,
                           norm_layer=norm_layer)  # 128
        self.de1 = DeLayer(output_nc, ngf, input_nc=input_nc,
                           norm_layer=norm_layer, outermost=True)  # 256
